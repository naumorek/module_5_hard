...
class UrTube:
    ...
    def log_in(self,nickname, password):
        ...
        for i in self.user_list:
            ...
            if i==nickname:
                ...
                if self.user_list[nickname][0]==password:
    ...
    def get_videos(self,search_str):
        ...
        for i in self.videos:
            if search_str.upper() in i.title.upper():
                list_film.append(i)
        return list_film
    ...
    def watch_video(self,name_film):
        if self.current_user==None:
            print("Войдите на платформу")
        else:
            for i in self.videos:
                if name_film in i.title:
                    if i.adult_mode:
                        if len(self.user_list)>1:
                            for j in self.user_list:
                                
                                if str(j)==self.current_user:


                                    age1 = self.user_list[self.current_user][1]
                                    if age1>=18:

                                        while i.time_now<i.duration:
                                            sleep(1)
                                            i.time_now=i.time_now+1
                                            print(f' секунда: {i.time_now}')
                                        print("Конец видео")
                                    else:
...
class Video:
    ...
    def __str__(self):
        return (f'Название: {self.title}, Длительность фильма: {self.duration}')
    def __repr__(self):
